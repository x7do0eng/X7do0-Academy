...
const DetailController = {
        ...
        document.getElementById('sidebar').innerHTML = `
            <div class="sticky top-24 space-y-4">
                <div class="academic-card p-4 md:hidden">
                    <button id="mobile-menu-toggle" class="w-full flex items-center justify-between text-academic-primary font-bold">
            ...
            return `
                                <div class="space-y-1">
                                    <button class="w-full flex items-center justify-between text-sm font-bold text-academic-primary group" onclick="this.nextElementSibling.classList.toggle('hidden')">
                                        <span>${this.localize(cat.label)}</span>
                                        <span class="text-[10px] opacity-50">${completedCount}/${catQuestions.length}</span>
                                    </button>
                                    <div class="${isActiveCat ? '' : 'hidden'} space-y-1 mt-2 ps-4 border-s" style="border-color:var(--border-soft);">
    ...
    render(q) {
      ...
      <div id="q-header" class="mb-8">
        ...
        <h1 class="text-3xl md:text-4xl font-bold text-academic-primary">${this.localize(q.title)}</h1>
      </div>

      <div id="q-prompt" class="mb-8">
        <div class="academic-card p-6">
            <div class="label mb-3">${i18n.t('python.question.prompt')}</div>
            <div class="text-academic-secondary leading-relaxed text-base">${this.localize(q.prompt)}</div>
        </div>
      </div>
                ...
                <div class="reveal-content">
                    <div class="section-surface">
                        <ul class="space-y-2 text-sm text-academic-primary">
                            ${this.localize(q.steps).map(step => `<li class="flex items-start gap-3"><span class="w-1.5 h-1.5 rounded-full mt-2 flex-shrink-0" style="background:var(--accent);"></span>${step}</li>`).join('')}
                        </ul>
                    </div>
                </div>
            </div>
      ...
      <div class="flex items-center justify-between mb-8">
        ...
        <button id="complete-btn" class="px-6 py-2 rounded-lg text-sm font-bold transition-all ${isCompleted ? '' : 'btn-accent'}">
      ...
      <div id="q-nav" class="grid grid-cols-2 gap-4 pt-8 border-t" style="border-color:var(--border);">
        <a href="${prev ? `./question.html?id=${prev.id}` : '#'}" class="${!prev ? 'opacity-30 pointer-events-none' : ''} academic-card p-4 transition-all ${!prev ? '' : 'hover:border-accent'}">
        ...
        <a href="${next ? `./question.html?id=${next.id}` : '#'}" class="${!next ? 'opacity-30 pointer-events-none' : ''} academic-card p-4 transition-all text-end ${!next ? '' : 'hover:border-accent'}">
